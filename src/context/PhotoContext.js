...
const PhotoContextProvider = props => {
  ...
  // flickr do not load more than 500 image per page.
  nbImagePerPage = nbImagePerPage > 500 ? 500 : nbImagePerPage;
  ...
  const runSearch = (query, mode) => {
    axios
      .get(
        `https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=${apiKey}&tags=${query}&page=${page}&per_page=${nbImagePerPage}&format=json&nojsoncallback=1`
      )
      .then(response => {
        if (mode === 'scroll-infinite') {
          setImages(images.concat(response.data.photos.photo));
        } else {
  ...
  return (
    <PhotoContext.Provider value={{ images, loading, imageExtraInfos, runSearch, runInfo, page, setPage }}>
      {props.children}
